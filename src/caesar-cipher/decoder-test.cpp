...
/*
   Test that checks if the updated word list has 90107 words, the correct number
   of words after it is filtered to include words with a length of 3 or more
   characters and includes words with no special characters.
*/
...
TEST(Decoder, DecodeTest) {
  std::string test_string =  "KHOOR ZRUOG";
  EXPECT_EQ(break_encrypt(create_dict("./words.txt"), test_string), 3); }