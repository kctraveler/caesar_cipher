...
/*
  Main driver method that takes in the encoded input and decrypts the input.
  Once the parameters are defined, the sentences are decrypted with the correct
  key. If user decides to write to a file, the decrypted sentences are written
  to file path along with their keys otherwise sentences are written to stdout.
